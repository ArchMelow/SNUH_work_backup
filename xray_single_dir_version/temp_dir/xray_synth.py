...
# +(2023.2.15) napari 라는 OpenGL 방식의 3d graphic viewer로 더 좋은 quality의 이미지 gen - 그 동안 만든 것보다 더 효율적이고 직관적이지만 만든것과는 상관없음.
...
def rotation3d(image, theta_x, theta_y, theta_z, output_spacing = None, background_value=0.0):
    ...
    return sitk.GetArrayFromImage(sitk.Resample(image, 
                         output_size, 
...
if __name__ == '__main__' :
    ...
    while True:
        ...
        if event == '폴더 선택' :
            ...
            if not patient_data :
                sg.Popup('사용할 수 없는 데이터입니다. Slice 수가 1200 보다 많거나, 아예 발견되지 않는 파일입니다.')
                continue
        ...
        if event == '설정 완료' :
            ...
            '''
            popup_choice = sg.PopupOKCancel('자체 회전 방법을 이용하시려면 OK, viewer를 이용하시려면 Cancel을 눌러주세요.')
            ...
            for label_file in tqdm(patient_data[values['_SELECTED_'][0]]['label_files']) :
            ...
            if popup_choice == None :
                ...
                sg.Popup('Viewer에서 저장하시면 자동으로 보정되어 결과값이 도출됩니다.\n회전 후 Contrast, Gamma 값을 알맞게 조정하는 것을 추천합니다.')
                viewer = napari.view_image(sitk.GetArrayFromImage(sitk_img_3d), name = 'xray')
                save_button = QPushButton('이미지와 각도 저장하고 나가기')
                viewer.window.add_dock_widget(save_button, area = 'left', name = 'save_and_exit')
                viewer.dims.ndisplay = 3 # 3 dimensional Display
                viewer.layers['xray'].rendering = 'average'

                xray_img = np.zeros((512,512))

                flag = 0

                def btn_trigger():
                    print('angles rotated : ', viewer.camera.angles)
                    xray_img = viewer.screenshot()
                    flag = 1
                    viewer.close()
                
                save_button.clicked.connect(btn_trigger)


                scaled_size = 512 if 512 > int(values['num_slices']) else int(values['num_slices'])

                xray1 = resize(xray_img, (scaled_size, scaled_size))

                xray2 = resize(xray_img, (scaled_size, scaled_size))
                ...
                img_cor = rotation3d(image = sitk_img_3d, 
                                          ...
                                          theta_y = theta_sg,
                                          theta_z = theta_ax, 
                                          ).transpose(1, 2, 0) # transpose 3d image to (x, y, z)
                ...
                xray1 = resize(np.sum(img_cor, axis = 0), (scaled_size, scaled_size))

                xray2 = resize(np.sum(img_cor, axis = 1), (scaled_size, scaled_size))
